...
public class CommandLineConfigsTests
{
    ...
    public static readonly TheoryData<Option, string, object> TestRemoveCommandAcceptTheoryData = new()
    {
        { CommandLineConfigs.UninstallAllOption, "", null },
        { CommandLineConfigs.UninstallAllLowerPatchesOption, "", null },
        { CommandLineConfigs.UninstallAllButLatestOption, "", null },
        { CommandLineConfigs.UninstallAllButOption, "2.2.300", new[] { "2.2.300" } },
        { CommandLineConfigs.UninstallAllButOption, "2.2.300 3.0.100", new[] { "2.2.300", "3.0.100" } },
        { CommandLineConfigs.UninstallAllBelowOption, "2.2.300", "2.2.300" },
        { CommandLineConfigs.UninstallAllPreviewsOption, "", null },
        { CommandLineConfigs.UninstallAllPreviewsButLatestOption, "", null },
        { CommandLineConfigs.UninstallMajorMinorOption, "2.2", "2.2" },
        { null, "2.2.300", new[] { "2.2.300" } }, 
        { null, "2.2.300 3.0.100", new[] { "2.2.300", "3.0.100" } },
        { null, "--unknown-option", new[] { "--unknown-option" } },
        { null, "--unknown-option argument", new[] { "--unknown-option", "argument" } }
    };

    [Theory]
    [MemberData(nameof(TestRemoveCommandAcceptTheoryData))]
    internal void TestRemoveCommandAccept(Option option, string argValue, object expected)
    {
        var parseResult = CommandLineConfigs.UninstallRootCommand.Parse($"remove {option?.Name} {argValue}");

        if (option is not null)
        {
            parseResult.CommandResult.GetValueForOption(option).Should().NotBeNull();
            parseResult.GetValueForOption(option).Should().BeEquivalentTo(expected);
        }
        else
        {
    ...
    [Theory]
    [MemberData(nameof(TestRemoveCommandAcceptTheoryData))]
    internal void TestDryRunCommandAccept(Option option, string argValue, object expected)
    {
    ...
    [Theory]
    [MemberData(nameof(TestRemoveCommandAcceptTheoryData))]
    internal void TestWhatIfCommandAccept(Option option, string argValue, object expected)
    {
        var parseResult = CommandLineConfigs.UninstallRootCommand.Parse($"whatif {option?.Name} {argValue}");
    ...
    [Theory]
    [MemberData(nameof(TestRemoveCommandAcceptTheoryData))]
    internal void TestGetUninstallRemoveOptionAccept(Option option, string argValue = "")
    {
        var commandResult = CommandLineConfigs.UninstallRootCommand.Parse($"remove {option?.Name} {argValue}").CommandResult;
        ...
        else
        {
            commandResult.GetUninstallMainOption().Name
                .Should().Be(option.Name);
        }
    }

    [Theory]
    [MemberData(nameof(TestRemoveCommandAcceptTheoryData))]
    internal void TestGetUninstallDryRunOptionAccept(Option option, string argValue = "")
    {
        var commandResult = CommandLineConfigs.UninstallRootCommand.Parse($"dry-run {option?.Name} {argValue}").CommandResult;
    ...
    [Theory]
    [MemberData(nameof(TestRemoveCommandAcceptTheoryData))]
    internal void TestGetUninstallWhatIfOptionAccept(Option option, string argValue = "")
    {
        var commandResult = CommandLineConfigs.UninstallRootCommand.Parse($"whatif {option?.Name} {argValue}").CommandResult;
